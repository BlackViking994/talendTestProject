	...
	public static class row2Struct implements
		...
		private String readString(ObjectInputStream dis) throws IOException {
			...
			if (length == -1) {
				...
				if (length > commonByteArray_LOCAL_PROJECT_DB.length) {
					if (length < 1024
							&& commonByteArray_LOCAL_PROJECT_DB.length == 0) {
						commonByteArray_LOCAL_PROJECT_DB = new byte[1024];
					} else {
						commonByteArray_LOCAL_PROJECT_DB = new byte[2 * length];
					}
				}
				dis.readFully(commonByteArray_LOCAL_PROJECT_DB, 0, length);
				strReturn = new String(commonByteArray_LOCAL_PROJECT_DB, 0,
						length, utf8Charset);
			}
			return strReturn;
		}
		...
		public void readData(ObjectInputStream dis) {
			...
			synchronized (commonByteArrayLock_LOCAL_PROJECT_DB) {
				...
				try {
					...
					this.DBPort = readInteger(dis);

					this.DBUser = readString(dis);

					this.DBPassword = readString(dis);

					this.DBSchema = readString(dis);

					this.inputFilePath = readString(dis);
		...
		public void writeData(ObjectOutputStream dos) {
			try {
				...
				writeString(this.DBPassword, dos);

				// String

				writeString(this.DBSchema, dos);

				// String

				writeString(this.inputFilePath, dos);
		...
		public String toString() {
			...
			sb.append(",DBPort=" + String.valueOf(DBPort));
			sb.append(",DBUser=" + DBUser);
			sb.append(",DBPassword=" + DBPassword);
			sb.append(",DBSchema=" + DBSchema);
			sb.append(",inputFilePath=" + inputFilePath);
			sb.append("]");
	...
	public void tFileInputXML_1Process(
		...
		try {
			...
			if (resumeIt || globalResumeTicket) { // start the resume
				...
				util_tLogRow_1.addRow(new String[] { "DBHost", "DBPort",
						"DBUser", "DBPassword", "DBSchema", "inputFilePath", });
				...
				try {
					filename_tFileInputXML_1 = "/Users/milossundic/Desktop/DBData.xml";
				...
				if (isValidFile_tFileInputXML_1) {
					...
					org.dom4j.XPath x_tFileInputXML_1 = doc_tFileInputXML_1
							.createXPath(nsTool_tFileInputXML_1
									.addDefaultNSPrefix("/ServerConfiguration"));
					x_tFileInputXML_1
					...
					for (org.dom4j.tree.AbstractNode temp_tFileInputXML_1 : nodeList_tFileInputXML_1) {
						...
						try {
							...
							if (xml_api_tFileInputXML_1
								...
								row1.DBPassword = str_tFileInputXML_1;
							}
							...
							if (xml_api_tFileInputXML_1
								...
								row1.DBSchema = str_tFileInputXML_1;
							}
							Object obj5_tFileInputXML_1 = xpaths_tFileInputXML_1
									.get(5).evaluate(temp_tFileInputXML_1);
							...
							if (xml_api_tFileInputXML_1
								...
								row1.inputFilePath = str_tFileInputXML_1;
							}
						...
						if (row1 != null) {
							...
							globalMap.put("DBPort", row1.DBPort);
							globalMap.put("DBUser", row1.DBUser);
							globalMap.put("DBPassword", row1.DBPassword);
							globalMap.put("DBSchema", row1.DBSchema);
							globalMap.put("inputFilePath", row1.inputFilePath);
							...
							if (row2.DBHost != null) { //
								row_tLogRow_1[0] = String.valueOf(row2.DBHost);

							} //

							if (row2.DBPort != null) { //
								row_tLogRow_1[1] = String.valueOf(row2.DBPort);

							} //

							if (row2.DBUser != null) { //
								row_tLogRow_1[2] = String.valueOf(row2.DBUser);

							} //

							if (row2.DBPassword != null) { //
								row_tLogRow_1[3] = String
										.valueOf(row2.DBPassword);

							} //

							if (row2.DBSchema != null) { //
								row_tLogRow_1[4] = String
										.valueOf(row2.DBSchema);

							} //

							if (row2.inputFilePath != null) { //
								row_tLogRow_1[5] = String
										.valueOf(row2.inputFilePath);
	...
	public int runJobInTOS(String[] args) {
		...
		try {
			...
			java.io.InputStream inContext = DB.class.getClassLoader()
					.getResourceAsStream(
							"local_project/db_0_1/contexts/" + contextStr
									+ ".properties");
			if (inContext == null) {
				inContext = DB.class.getClassLoader().getResourceAsStream(
		...
		if (false) {
			System.out.println((endUsedMemory - startUsedMemory)
					+ " bytes memory increase when running : DB");
		}
...
/************************************************************************************************
 * 61968 characters generated by Talend Open Studio for ESB on the June 1, 2018
 * 1:35:08 PM CEST
 ************************************************************************************************/
